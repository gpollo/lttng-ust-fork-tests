...
run_fork() {
	trap 'kill $BGPID' SIGINT
	if [[ -n $USE_VALGRIND ]]; then
		VALGRIND_OPTS="--log-file=run1.valgrind.log --leak-check=full --show-leak-kinds=all"
		LD_PRELOAD=liblttng-ust-fork.so valgrind $VALGRIND_OPTS ./fork-preload &
	else
		LD_PRELOAD=liblttng-ust-fork.so ./fork-preload &
	fi

	seq $1 | while read N; do
		sleep 1
		kill -SIGINT $!
		echo "Sending SIGINT #$1"
	done
